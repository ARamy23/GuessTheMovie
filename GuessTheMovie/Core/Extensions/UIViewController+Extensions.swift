...
extension UIViewController {
  ...
  func showError(error: ARError) {
    guard error.code?.isPresentable ?? true else { return }
    let message = error.text.emptyIfNil
    let indicatorView = SPIndicatorView(title: message, preset: .error)
    indicatorView.dismissByDrag = true
    indicatorView.present(duration: 3, haptic: .error)
  }

  func showSucessMessage(successMessage: String) {
    let indicatorView = SPIndicatorView(title: successMessage, preset: .done)
    indicatorView.dismissByDrag = true
    indicatorView.present(duration: 3, haptic: .success)
  }
}
